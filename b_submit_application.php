...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT email FROM B_isilaniformu WHERE B_id = ?";
    ...
    try {
        ...
        $mail->setFrom('gelisim.tolgakaratas@gmail.com', 'Yeni İş Yeri Önerisi');
        $mail->addAddress($ilan_mail); 
        $mail->addAttachment($upload_file);

        $mail->CharSet = 'UTF-8';  
        $mail->isHTML(true);
        $mail->Subject = 'Yeni İşe Alım Fikri';
        $mail->Body = "<h2>Yen İşyeri Önerisi</h2>
        <p><strong>E-posta:</strong> $email</p>
        <p><strong>Telefon Numarası:</strong> $telefon</p>
        <p><strong>İş yeri Bilgileri:</strong> $ilanYazisi</p>";
