...
$sql = "SELECT * FROM k_isilaniformu WHERE k_id = $ilanId AND kullanici_id = $userId";
...
<div class="container">
        <h1>İlan Güncelle</h1>
        <form action="ilanguncelle_islem.php?id=<?php echo $row['k_id']; ?>" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="k_id" value="<?php echo $row['k_id']; ?>">
            <label for="firmaAdi">Firma Adı:</label>
            <input type="text" id="firmaAdi" name="firmaAdi" value="<?php echo htmlspecialchars($row['firmaAdi']); ?>" required>

            <label for="mail">Mail:</label>
            <input type="email" id="mail" name="mail" value="<?php echo htmlspecialchars($row['mail']); ?>" required>
            ...
            <label for="onBilgi">Ön Bilgi:</label>
            <textarea id="onBilgi" name="onBilgi" required><?php echo htmlspecialchars($row['onBilgi']); ?></textarea>

            <label for="isDetayi">İş Detayı:</label>
            <textarea id="isDetayi" name="isDetayi" required><?php echo htmlspecialchars($row['isDetayi']); ?></textarea>
