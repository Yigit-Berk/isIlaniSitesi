...
$sql = "SELECT k_id, meslek, firmaAdi, sehir, tarih, calismaTuru, logo, onBilgi FROM k_isilaniformu";
...
if ($result === false) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<img src="Resimler/' . $row['logo'] . '" >';
            echo '<h2><a class="a" href="ilaninceleme.php?id=' . $row['k_id'] . '">' . $row['meslek'] . '</a></h2>';
            echo '<p>' ."Firma Adı :". $row['firmaAdi'] . '</p>';
            echo '<p>' ."Şehir :". $row['sehir'] . '</p>';
            echo '<p>' ."On Bilgi :". $row['onBilgi'] . '</p>';
